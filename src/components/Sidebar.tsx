...
function Sidebar({ counts }: { counts: any }) {
  ...
  const menuItems = useMemo(() => {
    ...
    const isAdmin = role === 'admin' || role === 'super_admin' || user?.email === 'hello@venturemond.com';
    if (isAdmin) {
      ...
      <aside className={cn(
        ...
        <div className="p-8 border-b border-border/50 flex flex-col items-center">
          ...
          <div className="mt-2 flex items-center gap-1.5 px-2 py-1 bg-muted rounded-full">
            <div className={cn("w-1.5 h-1.5 rounded-full", (role === 'admin' || role === 'super_admin') ? "bg-emerald-500" : "bg-amber-500")} />
            <span className="text-[10px] font-bold text-muted-foreground uppercase">
              {user?.email === 'hello@venturemond.com' ? 'ADMIN' : (role === 'admin' || role === 'super_admin') ? 'SUB-ADMIN' : 'USER'}
            </span>
          </div>
        </div>
